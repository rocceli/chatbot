...
#we have the read_only to false so that the chatbot learns from the user input as
def create_bot(name):
    ...
    return Bot
#function to train the bot on a variety of topics
...
#function to train the bot on custom data
...
def start_chatbot(Bot):
    print('\033c')
    print("Hello, I am Vantey. How can I help you")
    bye_list = ["bye vantey", "bye", "good bye vantey", "good bye"]
    while (True):
        user_input = input("me: ")
        if user_input.lower() in bye_list:
            print("Vantey: Good bye and have a nice day")
            break
        response = Bot.get_response(user_input)
        print("Vantey:", response)